...
def main():
    ...
    for i, datas in enumerate(dataset):
        for data in datas:
            ...
            pred = [inverse_transform(a.data).numpy().transpose((0, 2, 3, 1)) for a in model.pred]
            pred_data = np.stack(pred, axis=3)
            seq_batch = np.stack(data['imgs'], axis=3)
            ...
            for t in range(opt.K + opt.T):
                ...
                savedir = os.path.join(opt.save_dir, data['video_name'][0].split('.')[0] + data['start-end'][0])
                makedir(savedir)
    ...
    ssim_plot = os.path.join(opt.quant_dir, 'results_model=' + opt.name + '_' + opt.which_epoch + 'psnr.png')
